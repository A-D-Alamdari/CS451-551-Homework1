...
class Layout:
    ...
    def __init__(self, layout_text: list[str]) -> None:
        ...
        self.width: int = len(layout_text[0])
        self.height: int = len(layout_text)
        ...
        self.agent_positions: list[tuple[int | bool, tuple[int, int]]] = []
        self.num_ghosts: int = 0
        self.process_layout_text(layout_text)
        self.layout_text: list[str] = layout_text
        self.total_food: int = len(self.food.as_list())
        # self.initialize_visibility_matrix()

    def get_num_ghosts(self) -> int:
        return self.num_ghosts

    def initialize_visibility_matrix(self) -> None:
        global VISIBILITY_MATRIX_CACHE
        if ''.join(self.layout_text) not in VISIBILITY_MATRIX_CACHE:
            from pacman.game import Directions
            vecs: list[tuple[float, float]] = [(-0.5, 0), (0.5, 0), (0, -0.5), (0, 0.5)]
            dirs: list[str] = [Directions.NORTH, Directions.SOUTH, Directions.WEST, Directions.EAST]
                ...
                for y in range(self.height):
                    if not self.walls[x][y]:
                        for vec, direction in zip(vecs, dirs):
                            ...
                            while (next_x + next_y) != int(next_x) + int(next_y) or not self.walls[int(next_x)][
                                int(next_y)]:
                                vis[x][y][direction].add((next_x, next_y))
                                next_x, next_y = x + dx, y + dy
            self.visibility = vis
            VISIBILITY_MATRIX_CACHE[''.join(self.layout_text)] = vis
        else:
            self.visibility = VISIBILITY_MATRIX_CACHE[''.join(self.layout_text)]
    ...
    def get_random_corner(self) -> tuple[int, int]:
        poses: list[tuple[int, int]] = [(1, 1), (1, self.height - 2), (self.width - 2, 1),
                                        (self.width - 2, self.height - 2)]
        return random.choice(poses)

    def get_furthest_corner(self, pac_pos: tuple[int, int]) -> tuple[int, int]:
        poses: list[tuple[int, int]] = [(1, 1), (1, self.height - 2), (self.width - 2, 1),
                                        (self.width - 2, self.height - 2)]
        dist, pos = max([(manhattan_distance(p, pac_pos), p) for p in poses])
        return pos

    def is_visible_from(self, ghost_pos: tuple[float, float], pac_pos: tuple[float, float], pac_direction: str) -> bool:
...
def get_layout(name: str, back: int = 2) -> Optional[Layout]:
...
def try_to_load(fullname: str) -> Optional[Layout]:
