...
_canvas_tsize = 12
_canvas_tserifs = 0
_bg_color = '#000000'  # Background color (updated in begin_graphics)
_mouse_enabled = 0  # Mouse interaction flag
...
# Module-level event helper used as default for d_o_e parameters
_DONT_WAIT = _tkinter.DONT_WAIT


def _do_one_event(arg):
    if _root_window is not None:
        _root_window.tk.dooneevent(arg)
...
def _destroy_window(_event=None):
    sys.exit(0)
...
def clear_screen(_background=None):
...
def move_circle(canvas_id, pos, r, endpoints=None):
    global _canvas_x, _canvas_y
    ...
    if os.path.isfile('flag'):
        edit(canvas_id, ('extent', e[1] - e[0]))
    else:
        edit(canvas_id, ('start', e[0]), ('extent', e[1] - e[0]))
    move_to(canvas_id, x0, y0)


def edit(canvas_id, *args):
    _canvas.itemconfigure(canvas_id, **dict(args))
...
def change_text(canvas_id, new_text, font=None, size=12, style='normal'):
    _canvas.itemconfigure(canvas_id, text=new_text)
    if font is not None:
        _canvas.itemconfigure(canvas_id, font=(font, '-%d' % size, style))


def change_color(canvas_id, new_color):
    _canvas.itemconfigure(canvas_id, fill=new_color)
...
def keys_pressed(d_o_e=_do_one_event, d_w=_DONT_WAIT):
    d_o_e(d_w)
    if _got_release:
        d_o_e(d_w)
...
def remove_from_screen(canvas_id, d_o_e=_do_one_event, d_w=_DONT_WAIT):
    if _canvas is not None:
        _canvas.delete(canvas_id)
    d_o_e(d_w)
...
def _apply_coord_delta(item, dx, dy):
    """Apply (dx, dy) offset to all coordinate pairs of a canvas item."""
    if _canvas is None:
        return
    coords = _canvas.coords(item)
    new_coords = [coord + (dx if i % 2 == 0 else dy) for i, coord in enumerate(coords)]
    _canvas.coords(item, *new_coords)


def move_to(item, x, y=None, d_o_e=_do_one_event, d_w=_DONT_WAIT):
    if y is None:
        try:
            x, y = x
        except Exception:
    ...
    if _canvas is None:
        return
    current_x, current_y = _canvas.coords(item)[0:2]
    _apply_coord_delta(item, x - current_x, y - current_y)
    d_o_e(d_w)


def move_by(item, x, y=None, d_o_e=_do_one_event, d_w=_DONT_WAIT, lift=False):
    if y is None:
        try:
            x, y = x
        except Exception:
    ...
    _apply_coord_delta(item, x, y)
    d_o_e(d_w)
    if lift and _canvas is not None:
        _canvas.tag_raise(item)
...
if __name__ == '__main__':
    ...
    ghost_poly = polygon(ghost_shape, format_color(1, 1, 1))
    move_to(ghost_poly, (50, 50))
