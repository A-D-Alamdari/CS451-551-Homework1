...
from pacman.display.graphics_utils import *
import math
...
GHOST_COLORS = [
    format_color(.9, 0, 0),  # Red
    format_color(0, .3, .9),  # Blue
    format_color(.98, .41, .07),  # Orange
    format_color(.1, .75, .7),  # Green
    format_color(1.0, 0.6, 0.0),  # Yellow
    format_color(.4, 0.13, 0.91),  # Purple
]
...
class InfoPane:
    ...
    def initialize_ghost_distances(self, distances):
        ...
        for i, d in enumerate(distances):
            t = text(self.to_screen(self.width // 2 + self.width // 8 * i, 0), GHOST_COLORS[i + 1], d, "Times", size,
                     "bold")
    ...
    def set_team(self, is_blue):
        ...
        if is_blue:
            label = "BLUE TEAM"
        self.teamText = text(self.to_screen(300, 0), self.text_color, label, "Times", self.font_size, "bold")

    def update_ghost_distances(self, distances):
        if len(distances) == 0:
            return
        if not self.ghost_distance_text:
...
class PacmanGraphics:
    def __init__(self, zoom=1.0, frame_time=0.0, capture=False):
        ...
        # Attributes initialized in start_graphics / initialize
        self.is_blue = False
    ...
    def draw_static_objects(self, _state):
        layout = self.layout
    ...
    def draw_agent_objects(self, state):
        self.agent_images = []  # (agent_state, agent_img)
        for index, agent in enumerate(state.agent_states):
            if agent.is_pacman:
                agent_img = self.draw_pacman(agent, index)
                self.agent_images.append((agent, agent_img))
            else:
                agent_img = self.draw_ghost(agent, index)
                self.agent_images.append((agent, agent_img))
        refresh()
    ...
    def swap_images(self, agent_index, new_state):
        ...
        if new_state.is_pacman:
            agent_img = self.draw_pacman(new_state, agent_index)
            self.agent_images[agent_index] = (new_state, agent_img)
        else:
            agent_img = self.draw_ghost(new_state, agent_index)
            self.agent_images[agent_index] = (new_state, agent_img)
        refresh()

    def update(self, new_state):
        agent_index = getattr(new_state, '_agent_moved')
    ...
    def move_pacman(self, position, direction, agent_img):
        screen_position = self.to_screen(position)
        endpoints = self.get_endpoints(direction, position)
        r = PACMAN_SCALE * self.grid_size
        move_circle(agent_img[0], screen_position, r, endpoints)
        refresh()

    def animate_pacman(self, pacman, prev_pacman, agent_img):
        ...
        if self.frame_time > 0.01 or self.frame_time < 0:
            ...
            for i in range(1, int(frames) + 1):
                ...
                self.move_pacman(pos, self.get_direction(pacman), agent_img)
                refresh()
                sleep(abs(self.frame_time) / frames)
        else:
            self.move_pacman(self.get_position(pacman), self.get_direction(pacman), agent_img)
        refresh()
    ...
    def _direction_to_delta(direction):
        """Return (dx, dy) eye-offset for the given movement direction."""
        dx, dy = 0, 0
        if direction == 'North':
            dy = -0.2
        elif direction == 'South':
    ...
    def draw_ghost(self, ghost, agent_index):
        ...
        return [body, left_eye, right_eye, left_pupil, right_pupil]
    ...
    def move_eyes(self, pos, direction, eyes):
        ...
        move_circle(eyes[2], (screen_x + self.grid_size * GHOST_SIZE * (-0.3 + dx),
                              screen_y - self.grid_size * GHOST_SIZE * (0.3 - dy)), self.grid_size * GHOST_SIZE * 0.08)
    ...
    @staticmethod
    def remove_food(cell, food_images):
        x, y = cell
        remove_from_screen(food_images[x][y])

    @staticmethod
    def remove_capsule(cell, capsule_images):
        x, y = cell
    ...
    def update_distributions(self, distributions):
        ...
        distributions = map(lambda d: d.copy(), distributions)
        if self.distribution_images is None:
            self.draw_distributions(self.previous_state)
        for xi in range(len(self.distribution_images)):
            for yi in range(len(self.distribution_images[0])):
                dist_img = self.distribution_images[xi][yi]
                weights = [dist[(xi, yi)] for dist in distributions]
                ...
                for weight, gcolor in zip(weights, colors):
                    color = [min(1.0, c + 0.95 * gc * weight ** .3) for c, gc in zip(color, gcolor)]
                change_color(dist_img, format_color(*color))
        refresh()
